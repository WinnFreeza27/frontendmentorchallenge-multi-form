document.addEventListener('DOMContentLoaded', function() {
    ...
    function totalPayment() {
        const totalAddons = Object.values(data.addons).reduce((total, addon) => total + addon.price, 0);
    ...
    function checkPack(value, condition) {
        ...
        data.pack.name = packSelected.name;
        data.pack.subs = condition;
        data.pack.price = packSelected.price * (condition ? 10 : 1);
    }
    ...
    function updateFreeMonthsVisibility() {
        ...
        harga.forEach((element, index) => {
            const rawPrice = hargaData[index];
            const formatPrice = subsElement.checked ? rawPrice * 10 + "$/yr" : rawPrice + "$/mo";
            ...
            if (subsElement.checked) {
                if (!freeMonthsElement) {
                    ...
                    freeMonthsElement.classList.add('text-blue-950', 'text-sm');
    ...
    function updateLabelBackground(checkbox, label) {
        if (checkbox.checked) {
            label.classList.add('bg-gray-400/10');
        } else {
            label.classList.remove('bg-gray-400/10');
        }
    }

    function validateForm() {
        document.querySelectorAll('.error-message').forEach(msg => msg.classList.add('invisible'));
        ...
        if (!phoneInput.value.trim() || !isValidPhoneNumber(phoneInput.value.trim())) {
            document.getElementById('phone-error').classList.remove('invisible');
            valid = false;
        }
